...
def square_pad(sitk_img: sitk.Image, pad_value=0.0) -> sitk.Image:
    ...
    return sitk.ConstantPad(sitk_img, tuple(padding_lower), tuple(padding_upper), pad_value)
...
def transform_image(sitk_img: sitk.Image, reorientation_state, permutation: Tuple[int, int, int], flip_axes: Tuple[bool, bool, bool]):
    img = reorientation_state.apply(sitk_img[:])
